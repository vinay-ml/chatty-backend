...
export class PostCache extends BaseCache {
  ...
  public async savePostToCache(data: ISavePostToCache): Promise<void> {
    ...
    try {
      ...
      const postCount: string[] = (await this.client.HMGET(`users:${currentUserId}`, 'postsCount')) as string[];
  ...
  public async getPostsFromCache(key: string, start: number, end: number): Promise<IPostDocument[]> {
    try {
      ...
      for (const post of replies as IPostDocument[]) {
        ...
        post.createdAt = new Date(Helpers.parseJson(`${post.createdAt}`) as string);
  ...
  public async updatePostInCache(key: string, updatedPost: IPostDocument): Promise<IPostDocument> {
    ...
    try {
      ...
      postReply[0].createdAt = new Date(Helpers.parseJson(`${postReply[0].createdAt}`) as string);
